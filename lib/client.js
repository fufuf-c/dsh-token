...
/* ============ 会话内「本会话用量」:与 /dsh-token 同一套 iOS 设计语言 ============
   调色板直接照搬仪表盘的 :root / [data-theme=dark](数值一致,便于以后同步),
...
.dtk-app{
  ...
  --d-chip:rgba(120,120,128,.10); --d-surface2:rgba(120,120,128,.08);
  --d-text:#1d1d1f; --d-text2:#6e6e73; --d-text3:#aeaeb2;
  --d-hairline2:rgba(0,0,0,.07);
  --d-accent:#007aff; --d-accent-soft:rgba(0,122,255,.13);
  --d-green:#34c759; --d-red:#ff3b30; --d-blue:#007aff; --d-orange:#ff9500;
  --d-purple:#af52de; --d-teal:#5ac8fa; --d-indigo:#5856d6; --d-gray:#8e8e93;
  --d-shadow:0 1px 1px rgba(0,0,0,.03),0 2px 10px rgba(0,0,0,.05),0 14px 36px rgba(0,0,0,.07);
  background:var(--d-grad),var(--d-bg);
  color:var(--d-text);
  height:100%;box-sizing:border-box;overflow-y:auto;padding:14px 18px 24px;
  /* 字体与 DSH 外壳**完全一致**(同一个栈,含 "Segoe UI"):面板换了拉丁字体一眼就看得出来 */
  font:15px/1.47 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Hiragino Sans GB","Microsoft YaHei","Helvetica Neue",Helvetica,Arial,sans-serif;
...
body[data-ds-dark-theme] .dtk-app{
  ...
  --d-chip:rgba(120,120,128,.2); --d-surface2:rgba(120,120,128,.16);
  --d-text:#f5f5f7; --d-text2:#98989d; --d-text3:#6e6e73;
  --d-hairline2:rgba(255,255,255,.09);
  --d-accent:#0a84ff; --d-accent-soft:rgba(10,132,255,.22);
  --d-green:#30d158; --d-red:#ff453a; --d-blue:#0a84ff; --d-orange:#ff9f0a;
  --d-purple:#bf5af2; --d-teal:#64d2ff; --d-indigo:#7d7aff; --d-gray:#98989d;
...
.dtk-cat:not(:last-child)::after{content:'';position:absolute;left:32px;right:0;bottom:0;height:1px;background:var(--d-hairline2)}
...
`
    ...
    function SessionView(props) {
      ...
      if (noPrice) { metaBits.push(h('span', { key: 's3', className: 'sep' }, '·')); metaBits.push(h('span', { key: 'np' }, '含未定价模型,费用偏低')) }
