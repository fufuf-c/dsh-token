 ...
 *
 * - 数据管道(折叠/聚合/筛选/导出)全部在 ./core.mjs(纯函数,可单测);
...
import {
  ...
  kpiQuery, makeFilter, parseQuery,
} from './core.mjs'
...
export function apply(ctx) {
    ...
    try {
      if (existsSync(STORE_FILE)) {
        ...
        if (parsed && parsed.requests) {
          store = parsed
          if (store.version !== 3) {
            // v2 → v3:day 聚合新增逐会话索引;迁移标记一次全量扫描
            //(补齐老会话标题),之后的常规启动都走增量。
            store.version = 3
  ...
  function routeHandler(req, res) {
    ...
    const run = async () => {
      ...
      const result = apiDispatch(store, name, query, null, {})
  ...
  const api = {
    ...
    config: () => ({ prices: store.config.prices, budget: store.config.budget, webPagePath: store.config.webPagePath, retention: store.config.retention }),
    meta: () => apiDispatch(store, 'meta', {}, null, { metaExtra: { version: PKG.version, pagePath: (store.config && store.config.webPagePath) || DEFAULT_PAGE } }).body,
  }
