...
test('rebuildAll 逐请求按时段取档并汇总', () => {
  // FRI(10) 是北京 10:00(高峰),FRI(20) 是北京 20:00(空闲)。两者在宿主时区上
  // 可能落在不同的**本地日**(例如 UTC-4 下分别是前一日 22:00 与当日 08:00)——
  // 日/月桶是本地口径,所以这里断言"日桶之和",不假设两条记录同属一天。
  const store = storeWith([
  ...
  const daySum = Object.values(store.days).reduce((s, d) => s + d.totals.cost, 0)
  assert.equal(daySum, 3, '日桶成本之和 = 总成本')
  const monthSum = Object.values(store.months).reduce((s, m) => s + m.totals.cost, 0)
  assert.equal(monthSum, 3, '月桶成本之和 = 总成本')
  // 日键必须由记录时间戳算出来(本地口径),而不是写死字符串
  for (const r of [a, b]) assert.ok(store.days[dayKeyOf(r.t)], `日桶 ${dayKeyOf(r.t)} 应存在`)
})
